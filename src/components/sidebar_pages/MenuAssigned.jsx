...
function MenuAssigned(props) {
    ...
    useEffect(() => {
        ...
        fetch('src/data/db.json', {signal: abortConst.signal})
            ...
            .then(jsonData => {
                const data = jsonData.tasks.filter(task => task.type == "Assigned" && task.status !== "Completed" && task.author == props.USER_INFO.id);
                setTasks(data);
                setIsFetching(false);
            })
            .catch(err => {
        ...
        <>
            ...
            <div className="page">
                ...
                <div className="card-container">
                    ...
                    {tasks && tasks.length === 0 ? <div className='card card-plus' style={{textAlign: "center", fontSize: "2rem"}}>You have yet to assign tasks</div> : <></>}
